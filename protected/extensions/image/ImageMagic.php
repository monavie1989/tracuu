...
class ImageMagic {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(ImageMagic::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => ImageMagic::$allowed_types[$image_info[2]],
	 ...
	 * wish to use height as master dim, set $image->master_dim = ImageMagic::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: ImageMagic::NONE, ImageMagic::AUTO, ImageMagic::WIDTH, ImageMagic::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== ImageMagic::NONE AND
				    $value !== ImageMagic::AUTO AND
				    $value !== ImageMagic::WIDTH AND
				    $value !== ImageMagic::HEIGHT)
					return FALSE;
			break;
		}
